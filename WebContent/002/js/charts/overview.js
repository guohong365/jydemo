(function($) {
    ...
    DataSetBuilder.prototype.mergeOption = function(options) {
        $.extend(true, this.options, options);
    };
    DataSetBuilder.prototype.setOption = function(options) {
        if (options) {
            this.options = options;
        }
    };
    DataSetBuilder.prototype.getCompanies = function() {
        let names = [
            { name: "安宁公交" },
            { name: "东川公交" },
            { name: "元谋公交" },
    ...
    DataSetBuilder.prototype.getYears = function(fromYear, count) {
        let years = [];
    ...
    DataSetBuilder.prototype.getMonths = function(fromMonth) {
        let months = [
    ...
    DataSetBuilder.prototype.getDays = function(year, month, endDay) {
    ...
    DataSetBuilder.prototype.buildHistoryData = function() {
        let historyOption = {
            name: '',
            min: 60,
            max: 1200,
    ...
    DataSetBuilder.prototype.buildOthers = function() {
        var option = {
            name: '',
            min: 0,
            max: 100,
    ...
    DataSetBuilder.prototype.getColumnData = function(dataset) {
    ...
    DataSetBuilder.prototype.buildPieData = function(historyData, index) {
        var pieOption = {
            name: '',
            min: 5,
            max: 100,
    ...
    DataSetBuilder.prototype.buildDataSets = function(options) {
    ...
    Overview.prototype.buildHistoryLine = function(dataset) {
        const LINE_COUNT = 4;
        let option = {
            color: ['red', 'green', 'red', 'green'],
            tooltip: { trigger: 'axis', textStyle: { color: 'white' } },
            grid: { top: 20, left: '0%', right: '0%', bottom: '0%', show: true, containLabel: true },
    ...
    Overview.prototype.buildPie = function(dataset) {
        let option = {
    ...
    Overview.prototype.buildMap = function(datasets) {
        ...
        names.forEach(function(from) {
            names.forEach(function(to) {
                lineData.push({ coords: [from.value, to.value] });
            });
        });
        ...
        let lines = {
            ...
            lineStyle: { color: "rgba(49,198,249,1)", width: 1, opacity: 0.2, curveness: .3 },
            data: lineData
        };
        let option = {
            tooltip: { triggerOn: "mousemove", backgroundColor: "rgba(198,198,198,0.8)", color: 'white' },
            geo: {
                map: '530000-2',
                roam: true,
                silent: false,
    ...
    Overview.prototype.buildCharts = function() {
    ...
    //表格数据
    Overview.prototype.buildTables = function() {
        let colors = [
    ...
    Overview.prototype.updateTables = function(dataType) {
        let tables = [];
    ...
    Overview.prototype.updateCharts = function(dataType) {
        this._current = dataType;
        let _this = this;
    ...
    Overview.prototype.update = function(timeType) {
        if (!this._initialied) {
            this.init();
        }
