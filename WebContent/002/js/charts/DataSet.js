(function ($) {
    ...
    class DataSet {
        ...
        select(filter, dataBlock) {
            let block = DataSet.parseBlock(dataBlock);
        ...
        selectDistinct(filter, dataBlock) {
        ...
        static getGroupKeys(row, groupBy, tag) {
                ...
                case 'array':
                    if (groupBy.length > 0) {
                        for (let i = 0; i < groupBy.length; i++) {
                            key = DataSet.getGroupKeys(row, groupBy[i], {
                                value: 'array: '
                            });
        ...
        sum(column, groupBy, sumName) {
            ...
            let dimension = DataSet.getGroupKeys(this.dimension, groupBy);
